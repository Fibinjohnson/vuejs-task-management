   ...
   <div v-for="todos in activeTodos" :key="todos.id" class="parent ml-auto space-x-10 mr-auto px-3 py-1">
      <div class="flex flex-row">
        <div v-if="!todos.isCompleted" class="active  flex flex-row border border-1 h-14 rounded shadow shadow-sm w-full">
          ...
          <v-switch label="Mark as completed" class=" mr-12 pl-4" v-model="todos.isCompleted" v-on:change="onChecked(todos)" inset></v-switch>
        </div>
      </div>
    </div>
  </div>
...
import EditModal from './EditModal.vue'
     ...
     console.log(activeTodos.value,completedTodos.value,'active and completed ')
     ...
     onMounted( async()=>{
      try{
       ...
       if(res.statusText==="OK"){
        ...
        console.log(store.getters['active/editModal'](5)[0])
